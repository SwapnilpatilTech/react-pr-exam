...
import { deleteProduct } from '../store/productslice.js';
import ProductForm from './ProductForm.jsx';

const ProductTable = () => {
  const dispatch = useDispatch();
  const { items: products, loading, searchTerm, selectedCategory } = useSelector(state => state.products);
              ...
              {filteredProducts.map((product) => (
                <tr key={product.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      {product.image ? (
                        <img src={product.image} alt={product.title} className="h-12 w-12 object-cover rounded-md" />
                      ) : (
